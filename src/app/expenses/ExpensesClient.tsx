...
export default function ExpensesClient({
  ...
  const expenseColumns = [
    {
      ...
      cell: (e: Expense) => {
        const v = vehicles.find((v) => v.id === e.vehicleId);
        return <span className="font-semibold text-text-primary">{v?.registrationNumber}</span>;
      },
    },
    {
      header: "Category",
      cell: (e: Expense) => <span className="text-text-secondary">{titleCase(e.category)}</span>,
    },
    {
      header: "Amount",
      cell: (e: Expense) => (
        <span className="font-mono font-medium text-text-primary">{formatCurrency(e.amount)}</span>
      ),
    },
    {
      header: "Date",
      cell: (e: Expense) => <span className="text-text-secondary">{formatDate(e.date)}</span>,
    },
    {
      header: "Description",
      cell: (e: Expense) => (
        <div className="max-w-xs truncate text-text-secondary">
          {e.description || <span className="italic text-text-muted">No description</span>}
        </div>
      ),
    },
  ];
  ...
  const fuelColumns = [
    ...
    {
      header: "Liters",
      cell: (f: FuelLog) => <span className="font-mono text-text-secondary">{f.liters} L</span>,
    },
    {
      header: "Cost / L",
      cell: (f: FuelLog) => <span className="font-mono text-text-secondary">{formatCurrency(f.costPerLiter)}</span>,
    },
    {
      header: "Total Cost",
      cell: (f: FuelLog) => (
        <span className="font-mono font-medium text-text-primary">{formatCurrency(f.totalCost)}</span>
      ),
    },
    {
      header: "Odometer",
      cell: (f: FuelLog) => <span className="font-mono text-text-secondary">{f.odometerAtFill.toLocaleString()} km</span>,
    },
    {
      header: "Date",
      cell: (f: FuelLog) => <span className="text-text-secondary">{formatDate(f.date)}</span>,
    },
  ];
        ...
        <div className="flex items-center justify-between border-b border-border-default pb-4">
          <div className="flex gap-4">
            <button
              ...
              className={`text-lg font-semibold px-2 py-1 transition-colors border-b-2 ${
                activeTab === "expenses"
